...
<?php
    $queryEmplead = mysqli_query($connect_admin,"SELECT * FROM Empleados WHERE id = '".$_SESSION['id_user_valentina']."' ");
	...
	if($_POST["titulo"] != "" && $_POST["descripcion"] != ""  ){
		...
		//REGISTRO NUEVO
			...
			'".$_POST["titulo"]."', '". utf8_encode($_POST["descripcion"])."', '".$script."', '1', '".$_POST["tipo_multimedia"]."', '".$_POST["visibilidad"]."', '".$_POST["variable"]."', '1', '".$hoy."'  ) ");
    ...
    <div class="card">
		...
		<div class="card-body"> 
            ...
            <h3>Publicar Reto</h3>
      		...
      		<p>¿Quieres publicar un reto?, solo debes completar el formulario y dar clic en publicar.</p>
            ...
            <select name="variable" required="required" class="form-control" style="margin-bottom:15px; width:auto; ">
                ...
                <?php 
                    $queryVar = mysqli_query($connect_clima,"SELECT * FROM Variables ORDER BY nombre ASC ");
                    while($dataVar = mysqli_fetch_array($queryVar)){
                       echo '<option value="'.$dataVar["id"].'">'.$dataVar["nombre"].'</option>';
                    }
				?>
            </select>
            ...
            <textarea name="descripcion" required="required" rows="4" class="form-control" rows="5" style="height: auto; margin-bottom:15px" placeholder="Ingresa una descripción detallada y completa para el reto que deseas proponer..."><?php echo $data["descripcion"] ?></textarea> 
...
<script>
	function Tipo_Archivo(val){
		if(val == 1){
			$("#tipo_archivos").html('<div style="color: #ff0606; font-size: 13px;">* Para seleccionar varias imágenes diferentes de CTRL+click y seleccione los archivos a cargar.</div>');
