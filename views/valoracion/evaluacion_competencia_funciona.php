<?php
    ...
    if($_GET["eval"] != ""){
        ...
        WHERE id_empresa = '".$_SESSION['id_empresa']."' AND anio = '2021' AND id_evaluado = '".$dataTempV["id_empleado"]."' 
        ...
        if($qryValC->num_rows == 0){
            ...
            (id_empresa, anio, id_evaluado, id_evaluador, tipo, observaciones, estado, created_at) 
            VALUES 
            ( '".$_SESSION['id_empresa']."', '2021', '".$dataTempV["id_empleado"]."', '".$dataTempV["id_evaluador"]."', '".$dataTempV["tipo"]."', '', 1, '".$hoy."' )");
    ...
    if($_POST["id_competencia"]  != "" || $_POST["id_evaluado"]  != "" ){
        ...
        if($_POST["id_registro"] == ""){
            ...
            ( id_evaluado, id_evaluador, tipo_evaluador, id_competencia, observaciones,  created_at) 
            VALUES 
            ('".$_POST["id_evaluado"]."', '".$_POST["id_evaluador"]."', '".$_POST["tipo_evaluador"]."', '".$_POST["id_competencia"]."', '".$_POST["observaciones"]."', '".$hoy."' )");

            $id_tmp = mysqli_insert_id( $connect_valoracion ); 

            foreach( $_POST["comp"] as $comportamiento ){
               $partes = explode(",", $comportamiento); 

                mysqli_query($connect_valoracion,"INSERT INTO Competencias_Respuestas_Comportamientos 
                ( id_respuesta, id_competencia, id_comportamientos, calificacion, created_at) VALUES 
                ('".$id_tmp."', '".$_POST["id_competencia"]."',  '".$partes[0]."', '".$partes[1]."', '".$hoy."' )");  
        ...
        //ACTUALIZACION
            ...
            mysqli_query($connect_valoracion,"UPDATE Competencias_Respuestas SET observaciones = '".$_POST["observaciones"]."'
            ...
            foreach( $_POST["comp"] as $comportamiento ){
               $partes = explode(",", $comportamiento); 
                ...
                calificacion = '".$partes[1]."' WHERE id_comportamientos = '".$partes[0]."' AND id_respuesta = '".$_POST["id_registro"]."' 
                AND id_competencia =  '".$_POST["id_competencia"]."'  ");  
    ...
    if($_POST["observaciones_finales"] != ""){
    ...
    if($_POST["terminar_evaluacion_final"] != ""){
        mysqli_query($connect_valoracion,"UPDATE Competencias_Evaluaciones SET estado = '2'
        WHERE id = '".$evaludor."' ");
    ...
    $queryEvaluacion = mysqli_query($connect_valoracion,"SELECT * FROM Competencias_Evaluaciones WHERE id = '".$evaludor."'");
...
<div align="left" style="padding: 10px 0px;">
	<table width="100%">
    	<tr>
        	<td><h5 style="margin-top: 8px;"></i> Evaluación</h5></td>
            <td align="right">
...
foreach($perfiles as $prf){
    ...
    id_competencia = '".$dataComp["id"]."' AND id_evaluador = '".$dataEvaluacion["id_evaluador"]."' ORDER BY id DESC ");
...
<input type="hidden" name="id_evaluado" value="<?php echo $dataEvaluacion["id_empleado"]; ?>">
    ...
    <div class="card-body" align="center">
        ...
        <table width="100%">
            ...
            while($dataPreguntas = mysqli_fetch_array($queryPreguntas)){
                ...
                WHERE id_competencia = '".$dataComp["id"]."' AND id_comportamientos = '".$dataPreguntas["id"]."' AND id_respuesta = '".$dataRespTemp["id"]."' ORDER BY id DESC ");
                ...
                $check_true_3 = "";
                $check_true_4 = "";
                if($dataPregTemp["calificacion"] == 1){ $check_true_1 = "checked"; }
                if($dataPregTemp["calificacion"] == 2){ $check_true_2 = "checked"; }
                if($dataPregTemp["calificacion"] == 3){ $check_true_3 = "checked"; }
                if($dataPregTemp["calificacion"] == 4){ $check_true_4 = "checked"; }

                echo '
                        <tr>
                            <td style="padding-right: 15px;"><b>'.$count_int.'.</b> </td>
                            <td>'.$dataPreguntas["pregunta"].'</td>
                            <td width="50">1 <input type="checkbox" class="checkbox_list comp_'.$dataPreguntas["id"].'" name="comp[]" value="'.$dataPreguntas["id"].', 1" '.$check_true_1.' onClick="SelectCheck(this, '.$dataPreguntas["id"].')" ></td>
                            <td width="50">2 <input type="checkbox" class="checkbox_list comp_'.$dataPreguntas["id"].'" name="comp[]" value="'.$dataPreguntas["id"].', 2" '.$check_true_2.' onClick="SelectCheck(this, '.$dataPreguntas["id"].')" ></td>
                            <td width="50">3 <input type="checkbox" class="checkbox_list comp_'.$dataPreguntas["id"].'" name="comp[]" value="'.$dataPreguntas["id"].', 3" '.$check_true_3.' onClick="SelectCheck(this, '.$dataPreguntas["id"].')" ></td>
                            <td width="50">4 <input type="checkbox" class="checkbox_list comp_'.$dataPreguntas["id"].'" name="comp[]" value="'.$dataPreguntas["id"].', 4" '.$check_true_4.' onClick="SelectCheck(this, '.$dataPreguntas["id"].')" ></td>
                        </tr>
                ';
                $count_int++;
...
<div class="card" style="margin-bottom: 15px">
    <div class="card-body" align="center">
        ...
        <textarea class="form-control" name="observaciones_finales"><?php echo $dataEvaluacion["observaciones"]; ?></textarea>
        
        <div align="right" style="margin-top: 10px">
            <button type="submit" class="btn btn-success btn-sm">
                Guardar Observaciones y Comentarios Finales
            </button>
        </div>
