...
<?php
	...
	if($_POST["nombre"] != ""){
		if($_POST["id_competencia"] != ""){
			...
			VALUES 
			( '".$dtEmpleado['id_empresa']."', '".$_SESSION['anio']."', '".$_SESSION['ciclo']."', '".$_POST["nombre"]."', '".$_POST["definicion"]."', '".$_POST["id_tipo"]."', '".$hoy."', '".$hoy."' ) ");
			$id_l = mysqli_insert_id($connect);
			
			
			$i = 0;
			foreach ($_POST["id_nivel"] as &$nivel) {
				mysqli_query($connect_valoracion,"INSERT INTO Competencias_Niveles (id_empresa, anio, id_ciclo, id_competencia, id_nivel, definicion, created_at, update_at) 
				VALUES 
				( '".$dtEmpleado['id_empresa']."', '".$_SESSION['anio']."', '".$_SESSION['ciclo']."', '".$id_l."', '".$nivel."', '".$_POST["definicion"][$i]."', '".$hoy."', '".$hoy."' ) ");
...
<div align="right" style="display: none">
    <a href="<?php echo $url; ?>?pg=valoracion/replicas/competencias_replicar">
...
<table class="table table-bordered">
	...
	<?php
		$count = 1;
		$query = mysqli_query($connect_valoracion,"SELECT * FROM Competencias WHERE id_empresa = '".$dtEmpleado['id_empresa']."' AND anio = '".$_SESSION['anio']."' 
		...
		while($data = mysqli_fetch_array($query)){
			...
			<tr>
				...
				<td align="center">
					<button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target=".modal_competencia" onclick="Ficha_Competencia('.$data["id"].')">
			...
			while($dataNiveles = mysqli_fetch_array($queryNiveles)){
				...
				while($dataPreguntas = mysqli_fetch_array($queryPreguntas)){
					...
					<tr>
						...
						<td colspan="2">'.$dataPreguntas["pregunta"].'</td>
					</tr>
					';
