...
<?php
    ...
    if($_POST["corte"] != "" && $_POST["id_nivel_cargo"] != "" ){
        $queryCargosTmp = mysqli_query($connect_valentina,"SELECT * FROM Cargos 
        WHERE id_empresa = '".$_SESSION['id_empresa']."' AND nivel_cargo = '".$_POST["id_nivel_cargo"]."' ");
        while($dataCargosTmp = mysqli_fetch_array($queryCargosTmp)){
            
            mysqli_query($connect_valoracion,"UPDATE Perfiles_Cargos SET corte = '".$_POST["corte"]."' WHERE id_cargo = '".$dataCargosTmp["id"]."' ");
  ...
  <thead>
    ...
    <tbody>
        ...
        foreach($Array_Nivel_Cargo as $nivel){
            ...
            //CARGAMOS CARGOS
            $query = mysqli_query($connect_valentina,"SELECT * FROM Cargos WHERE id_empresa = '".$_SESSION['id_empresa']."' AND nivel_cargo = '".$nivel[0]."' ");
            while($data = mysqli_fetch_array($query)){
                
                $queryPerfil = mysqli_query($connect_valoracion,"SELECT * FROM Perfiles_Cargos 
                WHERE id_empresa = '".$_SESSION['id_empresa']."' AND id_ciclo = '".$_SESSION['ciclo']."' AND id_cargo = '".$data["id"]."' ");
                $dataPerfil = mysqli_fetch_array($queryPerfil);
                
                $corte = $dataPerfil["corte"];
                
                $lista_cargos .= $data["nombre"]."<br>"; 
                
                //CARGAMOS LOS EMPLEADOS
                $arrayEmpleados = array();
                $queryEmpleados = mysqli_query($connect_valentina,"SELECT * FROM Empleados 
                WHERE id_empresa = '".$_SESSION['id_empresa']."' AND estado = 1 AND id_cargo = '".$data["id"]."' ");
                while($dataEmpleados = mysqli_fetch_array($queryEmpleados)){
                }
