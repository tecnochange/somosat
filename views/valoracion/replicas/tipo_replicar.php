<?php
    ...
    if($_POST["guardar_replicar"] != ""){
        $queryEvalTmp = mysqli_query($connect_valoracion,"SELECT * FROM Tipos WHERE id_ciclo = '".$_POST["id_ciclo_anterior"]."' ");  
        while($dataEvalTmp = mysqli_fetch_array($queryEvalTmp)){
            ...
            VALUES 
            ('".$_SESSION['id_empresa']."', '".$_SESSION['anio']."', '".$_SESSION['ciclo']."', '".$dataEvalTmp["nombre"]."', '".$hoy."' , '".$hoy."' ) "); 
        }

        echo '<script> window.location = "'.$url.'?pg=valoracion/competencias_tipos"; </script>';
    }
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <table width="100%" style="margin-bottom: 10px">
                <tr>
                    <td>
                        <h2>Replicar Tipo de Competencias para el año <b><?php echo $_SESSION["anio"]; ?></b> de Ciclo: <b><?php echo $dataCicloVal["nombre"]; ?></b></h2>
                    </td>
                    <td align="right">
                        <a href="<?php echo $url; ?>?pg=valoracion/competencias_tipos">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-bordered table-sm" style="margin-top: 15px">
                        ...
                        <?php
                                                    //AND id_ciclo = '".$dataCicloAnterior['id']."'
                        $count = 1;
                        $query = mysqli_query($connect_valoracion,"SELECT * FROM Tipos WHERE id_empresa = '".$_SESSION['id_empresa']."' AND anio = '".$dataCicloAnterior['anio']."' 
                        ...
                        while($data = mysqli_fetch_array($query)){
                            ...
                            <tr>
                                ...
                                <td>'.$data["nombre"].'</td>
                                <td>'.$data["anio"].'</td>
                                <td align="center">'.$dataCl["nombre"].'</td>
                            </tr>
                            ';
                            $count++;
        ...
        <?php } else{ echo '<div align="center" style="width: 100%;">No hay ciclo anterior. Recuerde que para replicar primero debe existir un ciclo actual.</div>'; } ?>
