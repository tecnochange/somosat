<?php
	...
	if($_POST["nombre"] != ""){
		...
		if($_POST["id_registro"] != ""){
            ...
            $queryEps = mysqli_query($connect_valentina,"SELECT * FROM Eps WHERE id = '".$_POST["id_registro"]."' ");  
            $dataEps = mysqli_fetch_array($queryEps);
            
            $resumen = 'Edición Eps:\n';
            if($dataEps["nombre"] != $_POST["nombre"] ){ $resumen .= 'Nombre: '.$_POST["nombre"].'\n'; }
            if($dataEps["estado"] != $_POST["estado"] ){ $resumen .= 'Estado: '.$_POST["estado"].'\n'; }
            
            GuardarNovedad($connect_valentina, "Edición Eps", $_SESSION["id_user_valentina"], $resumen, $_POST["id_registro"] );
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            ...
            //CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            $resumen = 'Creación Eps:\n';
            if($dataEps["nombre"] != $_POST["nombre"] ){ $resumen .= 'Nombre: '.$_POST["nombre"].'\n'; }
            if($dataEps["estado"] != $_POST["estado"] ){ $resumen .= 'Estado: '.$_POST["estado"].'\n'; }
            
            GuardarNovedad($connect_valentina, "Creación Eps", $_SESSION["id_user_valentina"], $resumen, 0 );
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            ...
            echo '
            <script>
                window.location = "?pg=administrar/administrar/eps/eps";
            </script>
            ';
	...
	$query = mysqli_query($connect_valentina,"SELECT * FROM Eps WHERE id = '".$id."' ");
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="<?php echo $url; ?>?pg=administrar/administrar/eps/eps">Eps</a></li>
