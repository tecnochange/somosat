        ...
        <table class="table table-bordered">
            <thead class="thead-success">
                <tr>
                    ...
                    <th scope="col">Emergencia Médica</th>
                    <th scope="col">Estado</th>
                    <th scope="col" width="60" align="center">
                        <a href="<?php echo $url; ?>?pg=administrar/administrar/emergencia_medica/detalle">
                            <button type="button" class="btn btn-warning btn-sm" title="Nuevo">
            ...
            <tbody class="tabla_lista">
                <?php
                    $count = 1;
                    $query = mysqli_query($connect_valentina,"SELECT * FROM Emergencia_Medica ORDER BY nombre ASC ");  
                    while($data = mysqli_fetch_array($query)){ 

                        $queryCompania = mysqli_query($connect_valentina,"SELECT * FROM Emergencia_Medica WHERE nombre = '".$data["nombre"]."' ");  
                        ...
                        if($user_log["role"] == 1){
                            $bt_editar = '
                                <a href="'.$url.'?pg=administrar/administrar/emergencia_medica/detalle&id='.$data["id"].'">
