<?php
	...
	if($_POST["nombre"] != ""){
		...
		if($_POST["id_registro"] != ""){
            ...
            $querySubgerencia = mysqli_query($connect_valentina,"SELECT * FROM Subgerencia WHERE id = '".$_POST["id_registro"]."' ");  
            $dataSubgerencia = mysqli_fetch_array($querySubgerencia);
            
            $resumen = 'Edición Subgerencia:\n';
            if($dataSubgerencia["nombre"] != $_POST["nombre"] ){ $resumen .= 'Nombre: '.$_POST["nombre"].'\n'; }
            if($dataSubgerencia["estado"] != $_POST["estado"] ){ $resumen .= 'Estado: '.$_POST["estado"].'\n'; }
            
            GuardarNovedad($connect_valentina, "Edición Subgerencia", $_SESSION["id_user_valentina"], $resumen, $_POST["id_registro"] );
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            
			$sentencia = "
            UPDATE Subgerencia SET 
                nombre = '".$_POST["nombre"]."', 
                estado = '".$_POST["estado"]."'  
            ...
            //CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            $resumen = 'Creación Subgerencia:\n';
            if($dataSubgerencia["nombre"] != $_POST["nombre"] ){ $resumen .= 'Nombre: '.$_POST["nombre"].'\n'; }
            if($dataSubgerencia["estado"] != $_POST["estado"] ){ $resumen .= 'Estado: '.$_POST["estado"].'\n'; }
            
            GuardarNovedad($connect_valentina, "Creación Subgerencia", $_SESSION["id_user_valentina"], $resumen, 0 );
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
			//CODIGO PARA OBTENER LOS DATOS QUE SERÁN GUARDADOS EN LAS NOVEDADES
            
            
            $sentencia = "
            INSERT INTO Subgerencia ( 
                nombre, 
                estado, 
                created_at
            ) 
            ...
            echo '
            <script>
                window.location = "?pg=administrar/administrar/subgerencia/subgerencia";
            </script>
            ';
	...
	$query = mysqli_query($connect_valentina,"SELECT * FROM Subgerencia WHERE id = '".$id."' ");
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="<?php echo $url; ?>?pg=administrar/administrar/subgerencia/subgerencia">Subgerencia</a></li>
