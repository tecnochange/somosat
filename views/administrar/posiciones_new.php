    ...
    <table class="table table-bordered">
        ...
        <tbody class="tabla_lista">
            <?php
				...
				/*
				$sentencia = "
					SELECT Posiciones_Copia.id AS id, Posiciones_Copia.estado AS estado, Cargos_Copia.nombre AS cargo_nombre, Niveles_Cargo.nombre AS nivel_cargo, Areas_New.nombre AS area_nombre, Departamento.nombre AS departamento_nombre 
					FROM Posiciones_Copia 
					LEFT JOIN Cargos_Copia ON Cargos_Copia.id = Posiciones_Copia.id_cargo 
					LEFT JOIN Niveles_Cargo ON Niveles_Cargo.id = Posiciones_Copia.id_nivel_cargo 
					LEFT JOIN Areas_New ON Areas_New.id = Posiciones_Copia.id_area 
					LEFT JOIN Areas_New AS Departamento ON Areas_New.id = Posiciones_Copia.id_departamento 
				";
				*/
                $query= mysqli_query($connect_valentina,"SELECT * FROM Posiciones_View ");  
                while($data = mysqli_fetch_array($query)){
					...
					*/
                    ...
                    WHERE CARGO = '".$data["cargo"]."' AND NIVEL = '".$data["nivel_cargo"]."' AND ÁREA = '".$data["area"]."' AND DEPARTAMENTO_2 = '".$data["departamento"]."'  ");  
                    ...
                    if($user_log["role"] == 1){
                        $bt_editar = '
                            <a href="'.$url.'?pg=administrar/posicion/detalle&id='.$dataPosiciones["id"].'">
