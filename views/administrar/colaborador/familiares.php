        ...
        if($_POST["id_informacion"] != ""){
			...
			foreach( $_POST["nombre_hijo"] as $key => $hijo){
				...
				if( $_POST["id_hijo"][$key] != "" ){
					...
					$sentencia_hijo = " INSERT INTO Empleados_Hijos
						...
						VALUES 
						( '".$_SESSION["id_colaborador_edit"]."', '".$_POST["nombre_hijo"][$key]."', 
			...
			foreach( $_POST["parentesco_otros"] as $key => $hijo){
				...
				if( $_POST["id_familiar"][$key] != "" ){
					...
					VALUES 
					( '".$_SESSION["id_colaborador_edit"]."', '".$_POST["parentesco_otros"][$key]."', '".$_POST["otros"][$key]."', '".$_POST["nombre_otro"][$key]."', '".$_POST["tipo_documento_otros"][$key]."', '".$_POST["fecha_nace_otros"][$key]."', '".$hoy."' )
					";
					mysqli_query($connect_valentina, $sentencia_otro);
				}
			}
...
<!-- hijos -->               
				...
				<?php 
				$queryHijos = mysqli_query($connect_valentina,"SELECT * FROM Empleados_Hijos WHERE id_empleado = '".$_SESSION["id_colaborador_edit"]."' ");
				?>
				...
				<?php 
				$queryFamiliar = mysqli_query($connect_valentina,"SELECT * FROM Empleados_Familiares_Otros WHERE id_empleado = '".$_SESSION["id_colaborador_edit"]."' ");
...
function EliminarHijo(id){
	if(id > 0){
		...
		window.location = "<?php echo $url; ?>?pg=administrar/colaborador/familiares";
	}
}
...
function EliminarOtro(id){
	if(id > 0){
		jQuery.ajax({
			...
			type:'post',
			data: {id_registro: id, url:"<?php echo $url; ?>?pg=administrar/colaborador/familiares"},
			}).done(function (resp){
				$("#xscript").html(resp);
			})
			.fail(function(resp) {
